...
import { isSuburbInSydney } from "@/shared/sydney-geofence";
...
export default function SearchScreen() {
  ...
  const { isLandscape, isWide, columns } = useScreenLayout();
  ...
  const { data: userAddress } = trpc.profiles.getAddress.useQuery(undefined, { enabled: !!user });
  ...
  const userLat = userAddress?.latitude ? parseFloat(String(userAddress.latitude)) : undefined;
  const userLon = userAddress?.longitude ? parseFloat(String(userAddress.longitude)) : undefined;
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (maxPrice) count++;
    if (minRating) count++;
    if (maxDistanceKm) count++;
    if (availableFrom) count++;
    if (availableTo) count++;
    count += skills.length;
    return count;
  }, [minPrice, maxPrice, minRating, maxDistanceKm, availableFrom, availableTo, skills]);
    ...
    <ScreenContainer>
      <View className="flex-1">
        ...
        <View className="px-6 pt-6 pb-4">
          <Text className="text-3xl font-bold text-foreground mb-2">Find a Cat Sitter</Text>
          <View className="flex-row items-center mb-3 bg-primary/10 rounded-lg px-3 py-2">
            <IconSymbol name="location.fill" size={14} color={colors.primary} />
            <Text style={{ color: colors.primary, fontSize: 12, fontWeight: "600", marginLeft: 6 }}>Sydney Metro Area Only</Text>
          </View>
          ...
          <View className="flex-row items-center bg-surface rounded-xl px-4 py-3 border border-border mb-3">
            ...
            <TextInput
              ...
              placeholder="Enter Sydney suburb..."
              placeholderTextColor="#1F2937"
              value={suburb}
              onChangeText={setSuburb}
            />
            {suburb.length > 2 && !isSuburbInSydney(suburb) && (
              <Text style={{ color: colors.error, fontSize: 11, marginLeft: 4 }}>Not in Sydney</Text>
            )}
          </View>
        ...
        {/* Filters Panel */}
        {showFilters && (
          <ScrollView className="px-6 pb-4 border-b border-border" style={{ maxHeight: 420 }}>
            {/* Location Distance Filter */}
            <Text className="text-sm font-bold text-foreground mb-2">
              <Text>Maximum Distance</Text>
              {!userLat && (
                <Text className="text-xs text-muted font-normal"> (set your address in Profile)</Text>
              )}
            </Text>
            <View className="flex-row flex-wrap mb-4">
              {DISTANCE_OPTIONS.map((opt) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  style={!userLat && opt.value ? { opacity: 0.5 } : undefined}
                  disabled={!userLat && !!opt.value}
                >
                  <Text
                    className={`text-sm font-semibold ${
                      maxDistanceKm === opt.value ? "text-white" : "text-foreground"
                    }`}
                  >
                    {opt.label}
                  </Text>
            ...
            <Text className="text-sm font-bold text-foreground mb-2">Availability Dates</Text>
            
            {/* Quick Date Buttons */}
            <View className="flex-row flex-wrap mb-3">
              <TouchableOpacity
                className={`mr-2 mb-2 px-3 py-2 rounded-full ${
                  availableFrom === quickDates.tomorrow && !availableTo
                    ? "bg-primary"
                    : "bg-surface border border-border"
                }`}
                onPress={() => {
                  setAvailableFrom(quickDates.tomorrow);
                  setAvailableTo("");
                }}
                activeOpacity={0.7}
              >
                <Text
                  className={`text-xs font-semibold ${
                    availableFrom === quickDates.tomorrow && !availableTo ? "text-white" : "text-foreground"
                  }`}
                >
                  Tomorrow
                </Text>
              ...
              <TouchableOpacity
                className={`mr-2 mb-2 px-3 py-2 rounded-full ${
                  availableFrom === todayStr && availableTo === quickDates.nextWeek
                    ? "bg-primary"
                    : "bg-surface border border-border"
                }`}
                onPress={() => {
                  setAvailableFrom(todayStr);
                  setAvailableTo(quickDates.nextWeek);
                }}
                activeOpacity={0.7}
              >
                <Text
                  className={`text-xs font-semibold ${
                    availableFrom === todayStr && availableTo === quickDates.nextWeek ? "text-white" : "text-foreground"
                  }`}
                >
                  This Week
                </Text>
              ...
              <TouchableOpacity
                className={`mr-2 mb-2 px-3 py-2 rounded-full ${
                  availableFrom === todayStr && availableTo === quickDates.nextMonth
                    ? "bg-primary"
                    : "bg-surface border border-border"
                }`}
                onPress={() => {
                  setAvailableFrom(todayStr);
                  setAvailableTo(quickDates.nextMonth);
                }}
                activeOpacity={0.7}
              >
                <Text
                  className={`text-xs font-semibold ${
                    availableFrom === todayStr && availableTo === quickDates.nextMonth ? "text-white" : "text-foreground"
                  }`}
                >
                  This Month
                </Text>
              ...
              <TouchableOpacity
                className={`mr-2 mb-2 px-3 py-2 rounded-full ${
                  showDatePicker ? "bg-primary" : "bg-surface border border-border"
                }`}
              ...
              >
                <Text
                  className={`text-xs font-semibold ${
                    showDatePicker ? "text-white" : "text-foreground"
                  }`}
                >
                  Custom Dates
                </Text>
            ...
            {showDatePicker && (
              <View className="mb-4">
                <View className="flex-row items-center gap-3">
                  ...
                  <Text className="text-muted mt-4">to</Text>
                  <View className="flex-1">
            ...
            {(availableFrom || availableTo) && (
              <View className="flex-row items-center mb-4 bg-primary/10 rounded-lg px-3 py-2">
                <IconSymbol name="calendar" size={14} color={colors.primary} />
                <Text style={{ color: colors.primary, fontSize: 12, fontWeight: "600", marginLeft: 6, flex: 1 }}>
                  {availableFrom && availableTo
                    ? `${formatDateDisplay(availableFrom)} — ${formatDateDisplay(availableTo)}`
                    : availableFrom
                    ? formatDateDisplay(availableFrom)
                    : formatDateDisplay(availableTo)}
                </Text>
                <TouchableOpacity
                  onPress={() => { setAvailableFrom(""); setAvailableTo(""); }}
                  activeOpacity={0.7}
                >
              ...
              {viewMode === "list" ? (
                <View className="flex-1 px-6">
                  <Text className="text-sm text-muted mb-3">
                    ...
                    {availableFrom ? ` available ${formatDateDisplay(availableFrom)}${availableTo ? ` — ${formatDateDisplay(availableTo)}` : ""}` : ""}
                  </Text>
                  <FlatList
                    key={columns}
                    numColumns={columns}
                    ...
                    data={(() => {
                      ...
                      if (sortBy === "distance") {
                        return sitters.sort((a: any, b: any) => (a.distanceKm || 999) - (b.distanceKm || 999));
